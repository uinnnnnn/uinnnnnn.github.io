    ...
    html += `
      <div class="agenda-date-group" data-date="${dateStr}">
        ...
        ${items.map((b) => {
          ...
          return `
          <div class="agenda-event-row ${eff}" data-booking-id="${escapeHtml(b.BookingID)}">
...
function bookingActionButtons(b) {
  return `
    <button data-action="edit-detail" title="編輯預約內容">✎</button>
    <button data-action="edit-menu" title="更改狀態">●</button>
  `;
}
...
function wireActionButtons(root) {
  root.querySelectorAll("[data-action]").forEach((btn) => {
    btn.addEventListener("click", (e) => {
      ...
      const action = btn.dataset.action;
      if (action === "edit-menu") { openAgendaActionMenu(bookingId, btn); return; }
...
function openBookingModal(defaults, editingBooking) {
  ...
  document.getElementById("bk-deposit-paid").addEventListener("change", (e) => {
    ...
    if (e.target.checked && !fullyPaidChecked && statusSelect.value === "pending") {
      if (confirm('已勾選「已匯款訂金」，要順便把預約狀態改成「已付訂」嗎？')) {
...
function openBookingDetailModal(bookingId) {
  ...
  bookingDetailBody.innerHTML = `
    <button class="modal-close" id="booking-detail-close">✕</button>
...
async function submitBooking() {
  ...
  try {
    ...
    if (isEdit) {
      ...
      if (state.bookings.find((b) => b.BookingID === editingId).Status !== status) {
      ...
      await Api.post("createBooking", { customerId, customerName: name, phone, date, startTime, endTime, service, price, status, notes, cancelReason, ...moneyFields, ...customerFields });
      showToast("預約已建立");
    }
    closeBookingModal();
...
document.getElementById("bl-date-from").addEventListener("change", (e) => { state.blDateFrom = e.target.value; renderBookingList(); });
document.getElementById("bl-date-to").addEventListener("change", (e) => { state.blDateTo = e.target.value; renderBookingList(); });
document.getElementById("bl-category-select").addEventListener("change", (e) => { state.blCategory = e.target.value; renderBookingList(); });
...
document.getElementById("export-customers-csv-btn").addEventListener("click", () => {
  const header = ["姓名", "電話", "LINE/IG", "生日", "標籤", "首次到店", "最近到店", "到店次數", "累計消費", "目前儲值金", "備註"];
  const rows = state.customers
    .slice()
    .sort((a, b) => (a.Name || "").localeCompare(b.Name || ""))
    .map((c) => [c.Name, c.Phone, c.Contact, c.Birthday, tagLabel[c.Tag] || c.Tag, c.FirstVisitDate, c.LastVisitDate, c.VisitCount, c.TotalSpend, customerStoredValueBalance(c.CustomerID), c.Notes]);
  downloadCsv(`客戶名單_${fmtDate(new Date())}.csv`, [header, ...rows]);
});
...
tplChips.forEach((chip) => {
  chip.addEventListener("click", () => {
    ...
    tplChips.forEach((c) => c.classList.toggle("on", c === chip));
    const extraLabel = TPL_EXTRA_LABEL[activeTpl];
